		...
		//$("#setkenaikan").submit(function(e){
			$('#setkenaikan').validate({
				submitHandler: function(form) {
					$.ajax({
							type: "POST",
							data: $('#setkenaikan').serialize()+'&id_kelas='+$('#kelaskenaikan').val(),
...
<table  class="adddata" id="tablenaik">
	...
	foreach($siswa as $id_siswa=>$datasiswa){?>
        <tr> 
            ...
            <td class="naikkelas" style="text-align:center;"> 
				<? if($set=='kenaikan'){?>
				<select name="kelasuntuknaik[<?=$id_siswa?>]" class="kenaikan required">
					
					<? foreach($kelasuntuknaik as $kelas=>$datakelas){?>
						<option <? if($datakelas['id']==@$siswasudahnaik[$id_siswa]['id_kelas_siswa_det_jenjang']){ echo "selected";}?> value="<?=$datakelas['id']?>"><?=$datakelas['status']?><?//=$datakelas['nama']?></option>
					<? } ?>
				</select>
				<? }else{ ?>
