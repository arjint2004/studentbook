...
*******************************************************************************/
$xx=str_replace("view.php","",$_SERVER['SCRIPT_FILENAME']);
define('FILE',$xx);
//echo FILE.$_GET['image'];die();
//print_r(getImageSize(FILE.$_GET['image']));die();
include ('imageTransform.php');
$fl=explode("/",$_GET['image']);

foreach($fl as $fli){}

if($fli==''){
	$_GET['image']='upload/images/noimage.jpg';
}
switch ($_GET['mode']) {
	case 'rotate':
