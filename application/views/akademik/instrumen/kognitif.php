			...
			if(<? if(isset($_POST['id_pelajaran'])){}else{?>$frm.find('*[name=id_pelajaran]').is('.valid') && <? } ?> $frm.find('*[name=bab]').is('.valid') && $frm.find('*[name=judul]').is('.valid') && $frm.find('*[name=tanggal_kumpul]').is('.valid') && $frm.find('*[name=keterangan]').is('.valid')) {
				$.ajax({
					...
					success: function(msg) {
						...
						ajaxupload("<? echo base_url();?>akademik/instrumen/uploadfile/"+msg,"response","image-list","filekognitif");
...
<form method="post" name="kirimpr" enctype="multipart/form-data" id="kirimpr"  style="width:700px;" action="<? echo base_url();?>akademik/instrumen/kognitif/0/<?=$id_pelajaran?>">
