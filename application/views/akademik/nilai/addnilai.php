...
$(document).ready(function(){
		...
		$("#subjectnilaiaddnilai").each(function(){
...
//Submit Starts		   
		...
		$("#subjectnilaiaddnilai").submit(function(e){
			$frm = $(this);
			...
			if($frm.find('*[name=subject]').is('.valid') && $frm.find('*[name=id_kelas]').is('.valid') && $frm.find('*[name=id_pelajaran]').is('.valid')) {
				$.ajax({
					type: "POST",
					data: $(this).serialize()+'&'+$('form#nilai').serialize()+'&id_referensi=0',
		...
		$("select#kelas_add").change(function(e){
			...
			$.ajax({
				type: "POST",
				data: $("form#subjectnilaiaddnilai").serialize()+'&id_kelas='+$(this).val(),
...
<form action="<? echo base_url();?>akademik/nilai/addnilai" id="subjectnilaiaddnilai" name="subjectnilaiaddnilai" method="post" >
	<div class="addaccountclose" onclick="$('.addaccount').remove();"></div>
		...
		<table class="adddata">
			<tr>
				<th colspan="3" style="text-align:right;"><a title="" class="button small light-grey absenbutton" id="simpanabsensi" onclick="$('#subjectnilaiaddnilai').submit();"> Simpan </a></th>
			</tr>
			<tr>
