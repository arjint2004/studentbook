...
//			$(obinputsub2).val(''+subbaris+'_'+baris+'');
		...
		$("table.penghubungortutk tr td div.add_sub_2").live('click', function() {
			...
			var parsub2	='parsub2_'+barispar_sub+'_'+barispar;
			
			//tr3x=tr3x.replace("sub_3", "sub_3 ncsub3 "+classbaris_sub+" "+classbaris);
			tr3x=tr3x.replace("sub_3", "sub_3 ncsub3 "+classbaris_sub+" "+parsub2+" "+classbaris);
...
//				$(obinputsub2).val(''+barispar_sub+'.'+barispar+'.1');
				...
				//alert("table.penghubungortutk tr."+parsub2);
				var lastobj		= $("table.penghubungortutk tr."+parsub2).last();
				var subbaris	= $("table.penghubungortutk tr."+parsub2).attr('sub_baris');
				var baris		= $("table.penghubungortutk tr."+parsub2).length;
				baris++;
				$(lastobj).after(tr3x);
				$("table.penghubungortutk tr."+parsub2).last().attr('sub_baris',subbaris);
				$("table.penghubungortutk tr."+parsub2).last().attr('baris',barispar);
				$("table.penghubungortutk tr."+parsub2).last().attr('baris_sub',baris);
				$("table.penghubungortutk tr."+parsub2).last().addClass(parsub2);
				// naming input
				var obinputsub2=$("table.penghubungortutk tr."+parsub2).last().children('td').first().next().next().children('input');
...
//				$(obinputsub2).val(''+subbaris+'.'+barispar+'.'+baris+'');
		...
		$("table.penghubungortutk tr td div.add_sub_3").live('click', function() {
			...
			tr4x=tr4x.replace("sub_4", "sub_4 ncsub4 "+classbaris_sub+" "+parsub2+" "+classbaris+" "+classbaris_sub_sub);
			...
			if(typeof $("table.penghubungortutk tr."+classbaris_sub_sub).attr('class') === 'undefined'){
				...
				$(obinputsub2).attr('name','program['+barispar_sub+'][child]['+barispar_sub+'_'+barispar+'][child]['+barispar_sub+'_'+barispar+'_'+barispar_sub_sub+'][child]['+barispar_sub+'_'+barispar+'_'+barispar_sub_sub+'_1][nama]');
										...
										?>
										  <tr class="sub_2 ncls ncsub2 par_<?=$sub2[0]?> par0_<?=$sub2[0]?>" sub_baris="<?=$sub2[0]?>" baris="<?=$sub2[1]?>">
											  ...
											  <td><a href="#"  style="float:none;" title="" class="button small light-grey hapustk">X</a></td>
												...
												?>      
													<tr class="sub_3 ncsub3 par_<?=$sub3[0]?> parsub2_<?=$sub3[0]?>_<?=$sub3[1]?> par_<?=$sub3[0]?>_<?=$sub3[1]?> ncls" sub_baris="<?=$sub3[0]?>" baris="<?=$sub3[1]?>" baris_sub="<?=$sub3[2]?>">
														<td>&nbsp;</td>
														...
														?>                                   
															<tr class="sub_4 ncsub4 par_<?=$sub4[0]?> parsub2_<?=$sub4[0]?>_<?=$sub4[1]?> par_<?=$sub4[0]?>_<?=$sub4[1]?> par_<?=$sub4[0]?>_<?=$sub4[1]?>_<?=$sub4[2]?> ncls" sub_baris="<?=$sub4[0]?>" baris="<?=$sub4[1]?>" baris_sub="<?=$sub4[2]?>">
																<td>&nbsp;</td>
																<td style="width: 1%; border-right: medium none; padding: 2px ! important;"></td>
																<td style="border-right: medium none;"><input type="text" style="margin-left: 40px; width: 87%;" name4="" value="<?=$data_4['nama']?>" name="program[<?=$sub4[0]?>][child][<?=$sub4[0]?>_<?=$sub4[1]?>][child][<?=$sub4[0]?>_<?=$sub4[1]?>_<?=$sub4[2]?>][child][<?=$sub4[0]?>_<?=$sub4[1]?>_<?=$sub4[2]?>_<?=$sub4[3]?>][nama]"></td>
																<td></td>
																<td></td>
																<td></td>
																<td></td>
