												...
												success: function(msg) {
													$("#wait").remove();	
														$.ajax({
															...
															<? if(empty($afektif)){?>
															url: '<?=base_url('akademik/perencanaan/psikomotorik')?>',
															<? }else{ ?>
															url: '<?=base_url('akademik/perencanaan/sukses/AFEKTIF')?>',
															<? } ?>
															beforeSend: function() {
								...
								<table style="display:none;">
									<tr id="master">
										<td></td>
										<td >
											<select <? if(isset($_POST['id_pelajaran'])){echo'disabled';}?> style="float:left;width:100%;" class="selectfilter afektif" id="pelajaran_addafktf" name="id_pelajaranx[]">
										...
										<td ><textarea class="afektif" style="height:30px;" cols="30" name="afektif[]" ></textarea></td>
										<td ><a class="button small light-grey" onclick="hapus(this);" title="Hapus baris ini"> <span> Hapus </span> </a></td>
									</tr>
								</table>
								
								<form action="<?=base_url()?>akademik/perencanaan/afektif" method="post" id="catatangurudataform" style="width:700px;height:100%;">
								<table>
									<tbody>
										<tr>
											<td>
												<b>INDIKATOR PENILAIAN AFEKTIF</b>
											</td>
										</tr>
									</tbody>
								</table>
								...
								<table id="data">
										...
										<tbody>
											...
											<tr >
												...
												<td ><textarea style="height:30px;" cols="30" name="afektif[<?=$dataaff['id']?>]" class="afektif" ><?=$dataaff['indikator']?></textarea></td>
												<td >
